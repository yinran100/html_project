...
function initThisModel(){
	...
	trendModel.diagram = ko.dependentObservable(function () {	//当页
		...
		for(var i in this.note()){
			arr.push(dataUtils.getDiagram(arr,this.note()[i].numbers,12,0));
		}
		return arr;
	}, trendModel);
	trendModel.maindiagram = ko.dependentObservable(function (){	//今天的期，算最大遗漏使用
		...
		for (var i in gameArray.todaynote[datanum]){
			maindiagram.push(dataUtils.getDiagram(maindiagram,gameArray.todaynote[datanum][i].numbers,12,0));
		}
		return maindiagram;
	}, trendModel);
	trendModel.diagramHtml = ko.dependentObservable(function () {	//页面的分布二维数组html	
		...
		for (var i = 0; i < max_rowcount; i++) {
			if(i<this.note().length){
				...
				diastr+= '<td class=\"dia\">'+data.ds+'</td>';
				diastr+= '<td class=\"dia\">'+data.js+'</td>';
				diastr+= '<td class=\"dia\">'+data.zhi+'</td>';
				diastr+= '<td class=\"dia\">'+data.span+'</td>';
				diastr+= '<td class=\"dia\">'+data.chong+'</td>';
