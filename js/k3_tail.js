	...
	trendModel.diagramHtml = ko.dependentObservable(function () {	//页面的分布二维数组html	
		...
		for (var i = 0; i < max_rowcount; i++) {
			if(i<this.note().length){
				...
				$.each(this.diagram()[i], function(index, value) {
					if (parseInt(value)== 0) {
						if(parseInt(index)<6){
							...
							else diastr+= '<td class=\"dia'+(data.form==0?' back':' ')+'\"><div class=\"one\" style=\"'+csstext+':'+K_public.COLOR[data.form]+'">'+(index+1)+'</div></td>';
				...
				diastr+= '<td class=\"dia\"><div class=\"tail\" style=\"color:'+K_public.COLOR[data.form]+'">'+data.sum%10+'</div></td>';
