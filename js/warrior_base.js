...
function initThisModel(){
	...
	trendModel.diagram = ko.dependentObservable(function () {	//当页
		...
		for(var i in this.note()){
			...
			arr.push(dataUtils.getTrend(arr,num[0]-1,10,0).concat(dataUtils.getTrend(arr,num[1]-1,10,10),dataUtils.getDiagram(arr,num,10,20),sumdia(arr,da.ds,da.js,30)));
		}
		return arr;
	}, trendModel);
	trendModel.maindiagram = ko.dependentObservable(function (){	//今天的期，算最大遗漏使用
		...
		for (var i in gameArray.todaynote[datanum]){
			...
			maindiagram.push(dataUtils.getTrend(maindiagram,num[0]-1,10,0).concat(dataUtils.getTrend(maindiagram,num[1]-1,10,10),dataUtils.getDiagram(maindiagram,num,10,20),sumdia(maindiagram,da.ds,da.js,30)));
		}
		return maindiagram;
	}, trendModel);
	trendModel.diagramHtml = ko.dependentObservable(function(){	//页面的分布二维数组html	
		...
		for (var i = 0; i < max_rowcount; i++) {
			if(i<this.note().length){
				...
				diastr = '<td class=\"termN time\"></td><td class=\"state\" colspan="10" ></td>';
				for(var j=0;j<23;j++)	diastr+= '<td class=\"dia\"><div></div></td>';
			}else{
...
function sumdia(arr,ds,js,st){
	var start = st?st:0;
	var a = new Array(3);
	for (var i = 0; i < 3; i++) {
		if (i==0||(i==1&&ds%3==0)||(i==2&&js%3==0)) {
			a[i] = 0;
		} else {
			if (arr.length > 0) {
