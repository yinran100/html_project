		...
		for (var i = 0; i < max_rowcount; i++) {
			if(i<this.note().length){
				...
				$.each(this.diagram()[i], function(index, value) {
					if (parseInt(value) == 0) {
							diastr+= '<td class=\"dia '+(data.lian[index]==1?'back':'')+'\"><div class=\"'+(index+1==narr[0]?'one':'zero')+'\">'+(index+1>9?(index+1):'0'+(index+1))+'</div></td>';
					} else {
						if(parent.person_setting.showyilou==0)	diastr+= '<td class=\"dia\"><div>'+value+'</div></td>';
						else diastr+= '<td class=\"dia\"><div></div></td>';
					}
				});
				diastr+= '<td class=\"dia\">'+data.sum+'</td>';
				diastr+= '<td class=\"dia\">'+data.span+'</td>';
				diastr+= '<td class=\"dia\">'+data.ds+':'+(K_public.K10_WIN_NUMBER-data.ds)+'</td>';
				diastr+= '<td class=\"dia\">'+data.js+':'+(K_public.K10_WIN_NUMBER-data.js)+'</td>';
				diastr+= '<td class=\"dia\">'+data.zhi+':'+(K_public.K10_WIN_NUMBER-data.zhi)+'</td>';
			}else if(i==this.note().length){		//倒计时那一行
				data.day = 0;
				diastr = '<td class=\"termN time\"></td><td class=\"state\" colspan="8" ></td>';
				for(var j=0;j<25;j++)	diastr+= '<td class=\"dia\"><div></div></td>';
			}else{
				data.day = 0;
