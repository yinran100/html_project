...
function initThisModel(){
	...
	trendModel.diagram = ko.dependentObservable(function () {	//当页
		...
		for(var i in this.note()){
			var sum = maindata[parseInt(i)+this.page()*max_rowcount].sum-15;
			arr.push(dataUtils.getTrend(arr,sum,36,0));
		}
		return arr;
	}, trendModel);
	trendModel.maindiagram = ko.dependentObservable(function (){	//今天的期，算最大遗漏使用
		...
		for (var i in gameArray.todaynote[datanum]){
			maindiagram.push(dataUtils.getTrend(maindiagram,maindata[parseInt(i)+x].sum-15,36,0));
		}
		return maindiagram;
	}, trendModel);
