...
function initThisModel(){
	...
	trendModel.diagram = ko.dependentObservable(function () {	//当页
		...
		for(var i in this.note()){
			...
			arr.push(dataUtils.getTrend(arr,num[0]-1,12,0).concat(dataUtils.getTrend(arr,num[1]-1,12,12),dataUtils.getTrend(arr,num[2]-1,12,24)));
		}
		return arr;
	}, trendModel);
	trendModel.maindiagram = ko.dependentObservable(function (){	//今天的期，算最大遗漏使用
		...
		for (var i in gameArray.todaynote[datanum]){
			...
			maindiagram.push(dataUtils.getTrend(maindiagram,num[0]-1,12,0).concat(dataUtils.getTrend(maindiagram,num[1]-1,12,12),dataUtils.getTrend(maindiagram,num[2]-1,12,24)));
		}
		return maindiagram;
	}, trendModel);
	trendModel.diagramHtml = ko.dependentObservable(function () {	//页面的分布二维数组html	
		...
		for (var i = 0; i < max_rowcount; i++){
			if(i<this.note().length){
				...
				$.each(this.diagram()[i], function(index, value) {
					if (parseInt(value) <= 0) {		//********
						if(parseInt(index)<12){
							...
							diastr += '<td class=\"dia\"><div class=\"third\">'+(index-23>9?(index-23):'0'+(index-23))+'</div></td>';
						}
					} else {
				...
				for(var j=0;j<38;j++)	diastr+= '<td class=\"dia\"><div></div></td>';
			}else{
				data.day = 0;
